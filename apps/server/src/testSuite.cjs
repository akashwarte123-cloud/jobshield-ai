...
const TEST_CASES = [
  ...
  },
  {
    ...
    validate: (res) => {
      console.log(`   - Risk Level: ${res.data.riskLevel} (Expected: HIGH)`);
      console.log(`   - Risk Score: ${res.data.riskScore} / 100 (Expected: >= 65)`);
      const checkSignal = res.data.signals.find(s => s.label.toLowerCase().includes("check") || s.label.toLowerCase().includes("wire"));
  ...
  },
  {
    ...
    validate: (res) => {
      ...
      console.log(`   - Risk Score: ${res.data.riskScore} / 100`);
      const emailSignal = res.data.signals.find(s => s.label.toLowerCase().includes("email") || s.label.toLowerCase().includes("recruiter"));
